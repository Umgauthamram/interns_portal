    ...
    return (
        <div className="relative h-[330px] w-full group">
            ...
            <div
                ...
                style={{
                    background: `linear-gradient(135deg, rgba(255,255,255,1) 0%, rgba(255,255,255,0.95) 100%), linear-gradient(135deg, transparent 60%, ${p.colorHex}25 100%)`
                }}
            >
                ...
                <div className="flex items-end justify-between relative z-10 border-t border-gray-50 pt-6">
                    <div className="space-y-1">
                    </div>
...
export default function ProjectsPage() {
    ...
    const isReadOnly = formData.id && (formData.progress === 100 || formData.status === 'Rejected');

    const handleEdit = (project) => {
        if (project.progress === 100 || project.status === 'Rejected') {
            toast.error(project.status === 'Rejected'
                ? "Protocol Locked: Rejected proposals cannot be modified."
                : "Intelligence Locked: Completed deployments cannot be modified.");
            return;
        }
                                ...
                                {modalStep === 1 ? (
                                    ...
                                    <form onSubmit={handleSubmission} className="space-y-12 pb-8 px-2">
                                        ...
                                        <div className="pt-8 flex gap-6 sticky bottom-0 bg-white py-6 border-t border-gray-50 z-20">
                                            ...
                                            <button
                                                type="submit"
                                                disabled={!formData.title || isReadOnly}
                                            ...
                                            >
                                                {formData.id ? "Sync Changes" : "Submit"}
                                            </button>
                                        </div>
                                    </form>
                                )}
