                        ...
                        >
                            ...
                            <div className="space-y-12">
                                <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                                    <h1 className="text-4xl font-black text-gray-900 tracking-tighter">Livebooks ({courses.length})</h1>
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                                    {[
                                        { label: "Livebooks", count: courses.length, icon: Brain, color: "text-amber-500", bg: "bg-amber-50" },
                                        { label: "Total Modules", count: courses.reduce((acc, c) => acc + (c.modules?.length || 0), 0), icon: Layers, color: "text-purple-500", bg: "bg-purple-50" },
                                        { label: "Published Topics", count: topics.length, icon: BookOpen, color: "text-emerald-500", bg: "bg-emerald-50" },
                                        { label: "Content Units", count: topics.reduce((acc, t) => acc + (t.blocks?.length || 0), 0), icon: Zap, color: "text-blue-500", bg: "bg-blue-50" },
                                    ].map((stat, i) => (
                                        <div key={i} className="bg-white border border-gray-100 p-6 rounded-[2rem] shadow-sm flex items-center gap-5 group hover:shadow-xl transition-all">
                                            <div className={`p-4 rounded-2xl ${stat.bg} ${stat.color} transition-all group-hover:scale-110 shadow-sm border border-transparent group-hover:border-gray-100`}>
                                                <stat.icon className="w-6 h-6" />
                                            </div>
                                            <div>
                                                <p className="text-[20px] font-black text-gray-900 leading-none">{stat.count}</p>
