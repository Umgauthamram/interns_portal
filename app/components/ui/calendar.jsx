        ...
        <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
                <Button
                    variant="outline"
                    aria-expanded={open}
                    className={cn('w-40 justify-between capitalize bg-gray-50 hover:bg-gray-100', className)}
                >
                    {value
            ...
            <PopoverContent className="w-48 p-0 bg-white border border-gray-100 shadow-2xl rounded-2xl overflow-hidden z-[110]">
                <Command
                    filter={(value, search) => {
...
export const CalendarBody = ({ features, children }) => {
    ...
    return (
        <div className="grid flex-grow grid-cols-7 h-auto rounded-md overflow-hidden bg-white">
            {days.map((day, index) => (
                <div
                    key={index}
                    className={cn(
                        'relative h-24 overflow-hidden border-t border-r border-gray-100 hover:bg-gray-50 transition-colors',
    ...
    return (
        <div className={cn('grid flex-grow grid-cols-7', className)}>
            {daysForLocale(locale, startDay).map((day) => (
                <div key={day} className="p-3 text-right text-muted-foreground text-xs font-semibold">
                    {day}
                </div>
            ))}
        </div>
...
export const CalendarItem = ({ feature, className }) => (
    <div className={cn('flex items-center gap-2 p-1 rounded hover:bg-neutral-100/50 cursor-default', className)} key={feature.id}>
        <div
        ...
        />
        <span className="truncate text-[10px] font-medium">{feature.name}</span>
    </div>
);
